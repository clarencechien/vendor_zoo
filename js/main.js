...
const SYS_TEXT = {
  ...
  "sys.cards_maxed": () => `本季手牌已出滿 ${CONFIG.cards_per_season} 張了。`,
  "sys.hand_empty": () => "手牌空。",
  ...
  "sys.event_dud": () => "（事件撲空，逃過一劫。）",
  "sys.ransom_paid": (v) => `→ 付了 $${v.amount}萬 贖回。`,
...
async function askDecision(req) {
  ...
  if (req.decision === "explode") {
    return card({
      ...
      choices: [
        { label: "硬扛認賠", sub: "賠錢，但罪孽不增", value: "eat" },
